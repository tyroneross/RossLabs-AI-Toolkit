...
|--------|---------|--------------|
| [build-loop](https://github.com/tyroneross/build-loop) | `0.10.0` (v0.11.0 pending) | Orchestrated 5-phase dev loop — Assess → Plan → Execute → Review → Iterate, plus optional Learn. Opus 4.7 orchestrator with debugger, tracer, and PRD bridges. **v0.11 (pending) ships native architecture awareness**: a Python-native engine (scan/impact/trace/rules/dead/lessons) under `src/build_loop/architecture/` with NavGator demoted from hard dep to optional escalation adapter; aggressive freshness via Edit/Write hooks (extension-allowlisted); Architecture Context Pack (ACP) injected into every phase; `architecture-scout` read-only subagent dispatched at Phase 1 baseline / Phase 2 chunk-impact fan-out / Phase 4-D rules / Phase 5 iterate-subgraph / Phase 6 learn-sync. A capability registry + ≤8 shortlist keeps orchestrator decision space below the tool-selection ceiling (~78 → ≤8 per phase, with trigger-aware demotion + plugin-surface collapse); a unified `recall()` facade fans across runs/decisions/semantic/debugger backends with graceful degradation. Phase 1 grounds every build in the project's strategic frame (`docs/prd-*.md`) and detects `riskSurfaceChange` triggers; Phase 2 plan-verify gate catches unverified delete/orphan claims, route changes, numeric drift, missing security acceptance criteria, and **schema-migration writer/reader chain drift**. v0.8 adds a Phase 4-A build-time security review against OWASP LLM/Agentic Top 10 + MITRE ATLAS + NIST AI 600-1, plus a `security_finding` recurrence detector that auto-proposes project-local rules when the same risk ID lands across 3+ builds. v0.9 adds DOE-first multi-factor optimization (`build-loop:optimize`) with autoresearch baseline handoff and a renamed `build-loop-debugger` MCP server. **v0.10 introduces a global path/schema resolver + `project_resolver` and migrates decision capture from per-repo `.episodic/decisions/` to a global `build-loop-memory` private store with `personal_memory` Postgres schema; lessons survive project deletion and are scoped via project tags.** UI builds get design-rule gates and visual validation (IBR/simulator). v0.6+ bundles debug-loop, debugging-memory, and logging-tracer skills + an incident-memory MCP server. |
