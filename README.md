...
|--------|---------|--------------|
| [build-loop](https://github.com/tyroneross/build-loop) | `0.6.0` | Orchestrated 5-phase dev loop — Assess → Plan → Execute → Review → Iterate, plus optional Learn. Opus 4.7 orchestrator with NavGator, debugger, tracer, and PRD bridges. Phase 1 grounds every build in the project's strategic frame (`docs/prd-*.md`); Phase 2 plan-verify gate catches unverified delete/orphan claims, route changes, and numeric drift. UI builds get design-rule gates and visual validation (IBR/simulator). v0.6 bundles debug-loop, debugging-memory, and logging-tracer skills + an incident-memory MCP server, so the full debugging workflow runs without a separate claude-code-debugger install. |
