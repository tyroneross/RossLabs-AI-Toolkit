...
|--------|---------|--------------|
| [build-loop](https://github.com/tyroneross/build-loop) | `0.10.0` | Orchestrated 5-phase dev loop — Assess → Plan → Execute → Review → Iterate, plus optional Learn. Opus 4.7 orchestrator with debugger, tracer, and PRD bridges. **Native architecture awareness**: a Python-native engine (scan/impact/trace/rules/dead/lessons) under `src/build_loop/architecture/` with NavGator demoted from hard dep to optional escalation adapter (auto/native/navgator modes); aggressive freshness via SessionStart + PreToolUse Edit/Write hooks (extension-allowlisted, single-flight, doc-edits never fire scans); Architecture Context Pack (ACP) injected into subagent briefs at Phase 2/3/4/5; `architecture-scout` read-only Sonnet subagent dispatched at Phase 1 baseline / Phase 2 chunk-impact fan-out / Phase 4-D rules / Phase 5 iterate-subgraph / Phase 6 learn-sync. A capability registry (116 entries × 6 kinds × 10 categories) + ≤8 shortlist with plugin-surface collapse + trigger-aware demotion keeps orchestrator decision space below the empirical tool-selection ceiling; Phase 1 invocation is mandatory and caches into `state.json.activeCapabilities` so Phase 2/3 dispatchers inherit instead of re-scoring. A unified `recall()` facade fans across runs / decisions (legacy + canonical build-loop-memory store) / Postgres `semantic_facts` / debugger MCP backends with graceful degradation; `backend_health` probe surfaces store status each Phase 1 and is consumed by Phase 5 Iterate to short-circuit unavailable backends. Phase 1 grounds every build in the project's strategic frame (`docs/prd-*.md`) and detects `riskSurfaceChange` triggers; Phase 2 plan-verify gate catches unverified delete/orphan claims, route changes, numeric drift, missing security acceptance criteria, and **schema-migration writer/reader chain drift**. v0.8 adds a Phase 4-A build-time security review against OWASP LLM/Agentic Top 10 + MITRE ATLAS + NIST AI 600-1, plus a `security_finding` recurrence detector that auto-proposes project-local rules when the same risk ID lands across 3+ builds. v0.9 adds DOE-first multi-factor optimization (`build-loop:optimize`) with autoresearch baseline handoff and a renamed `build-loop-debugger` MCP server. **v0.10 introduces a global path/schema resolver + `project_resolver` and migrates decision capture from per-repo `.episodic/decisions/` to a global `build-loop-memory` private store with `personal_memory` Postgres schema; lessons survive project deletion and are scoped via project tags.** UI builds get design-rule gates and visual validation (IBR/simulator). v0.6+ bundles debug-loop, debugging-memory, and logging-tracer skills + an incident-memory MCP server. |
