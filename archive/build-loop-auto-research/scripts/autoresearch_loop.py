...
"""autoresearch_loop.py — Core loop mechanics for the build-loop-auto-research plugin.
...
def load_experiment(workdir: Path) -> dict:
    """Load experiment.json from the .build-loop-auto-research directory."""
...
def load_results(workdir: Path) -> list[dict]:
    """Load results.tsv into a list of dicts keyed by column name."""
...
def init_experiment(workdir: Path, config: dict) -> Path:
    ...
    # Run baseline metric from target repo.
    ...
    # Wipe and re-create results.tsv with the baseline row.
...
def run_iteration(workdir: Path, iteration: int) -> IterationResult:
    ...
    2. Compute delta from the experiment baseline.
    ...
    # Measure metric (run from target repo).
    ...
    # Determine status — compare against running best, not baseline.
...
def check_convergence(workdir: Path) -> tuple[bool, str]:
    ...
    """
    experiment = load_experiment(workdir)
    budget = experiment.get("budget", 20)
    total = experiment.get("iterations_total", 0)

    # Budget check — total iterations (kept + discarded + errors), not just kept.
    if total >= budget:
        return True, "budget"
    ...
    # Skip the baseline row (iteration 0).
...
def get_experiment_summary(workdir: Path) -> dict:
    ...
    # Top 3 kept changes by delta descending.
    top_changes: list[dict] = []
    for row in sorted(kept, key=lambda r: float(r.get("delta", "0") or "0"), reverse=True)[:3]:
        try:
            top_changes.append(
                {
...
def _cli() -> None:
    ...
    parser = argparse.ArgumentParser(description="autoresearch loop mechanics")
    ...
    parser.add_argument("--delta", type=float, help="delta from baseline (for --log)")
    ...
    if args.init:
        ...
        for field in ("iteration", "commit", "metric", "delta", "status", "description"):
            if getattr(args, field) is None:
                print(f"ERROR: --{field} required for --log", file=sys.stderr)
                sys.exit(1)
        result = IterationResult(
